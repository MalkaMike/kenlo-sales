...
export function SegurosAddonCard() {
  ...
  const { cardRef, activeClass } = useAddonPulse(addons.seguros);

  return (
    <div
      ref={cardRef}
      className={`p-3 rounded-lg border transition-all duration-200 ease-in-out hover:scale-[1.02] hover:shadow-md cursor-pointer ${!isAddonAvailable("seguros") ? "opacity-50 bg-gray-50" : activeClass}`}
    >
